document.addEventListener('DOMContentLoaded', () => {
  ...
  if (statusEl) {
    ...
    const isSatToThurs = (day >= 6 || day <= 4);
    const open = isSatToThurs && h >= 9.5 && h < 20;
    
    if (open) {
      statusEl.innerHTML = '<div class="status-badge"><div class="status-dot"></div>Open Now — Closes 8 PM</div>';
    } else if (isSatToThurs && h < 9.5) {
      statusEl.innerHTML = '<div class="status-badge"><div class="status-dot"></div>Opens today at 9:30 AM</div>';
    } else if (!isSatToThurs) {
      statusEl.innerHTML = '<div class="status-badge"><div class="status-dot"></div>Closed — Opens Saturday 9:30 AM</div>';
    } else {
      statusEl.innerHTML = '<div class="status-badge"><div class="status-dot"></div>Closed — Opens tomorrow 9:30 AM</div>';
    }
  }
